...
Lerna is used as monorepo management tool. To take advantage of it, It can be installed globally by running ``` npm install -g lerna ```. After running that, ```npx nx graph``` can be used to visualize the dependency linking in the monorepo as such <img src="/docs/assets/img/nx-graph-example.png" alt="visualization of nx graph" style="height: 122px; width:304px;"/>
