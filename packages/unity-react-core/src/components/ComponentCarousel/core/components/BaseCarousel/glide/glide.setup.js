...
function buildConfig(perView = 1, isFullWidth, hasPeek = true, isDraggable) {
  ...
  return {
    ...
    focusAt: 0,
    bound: false, // Only if type slider with focusAt 0 // Set to false for accessibility (all cards should be accessible with a keyboard)
...
function updateNonVisibleSlides(gliderElement, currentIndex, perView) {
  ...
  // );
  const endVisibleIndex = currentIndex; // Don't skip to the last visible card when navigating to the right
