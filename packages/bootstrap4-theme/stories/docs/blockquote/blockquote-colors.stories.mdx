...
The open quote glyph that is included in the markup is a direct SVG element (as opposed to an image file) so that the color can be manipulated by a CSS rule.
