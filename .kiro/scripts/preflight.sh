...
MCP_JSON="$REPO_ROOT/.kiro/settings/mcp.json"
...
if [ -f "$MCP_JSON" ]; then
  if jq empty "$MCP_JSON" >/dev/null 2>&1; then
    ...
    servers="$(jq -r '.mcpServers | keys[]' "$MCP_JSON" 2>/dev/null | tr '\n' ' ')"
...
echo "-- required env"
check_env_required JIRA_API_TOKEN "needed for Jira intake via mcp-atlassian"
if [ "$STAGE" = "unity" ]; then
