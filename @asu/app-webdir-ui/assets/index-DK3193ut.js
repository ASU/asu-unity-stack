import{j as e}from"./jsx-runtime-DvmJwJjP.js";import{P as r,q as A}from"./styled-components.browser.esm-BecGWgJO.js";import{r as l}from"./index-DUDtsY9M.js";import{a as C,e as c,J as V,h as K,A as B,K as $,S as M}from"./sort-C7jqMW2L.js";function z({packageName:n="",component:u="",type:f="",configuration:y={}}){if(!n||!u){console.error("trackReactComponent: Missing required arguments.");return}window.uds=window.uds||{},window.uds.package=window.uds.package||{},window.uds.package[n]={component:u,type:f,configuration:y}}const F={1:"Faculty",2:"Academic Professionals",3:"Other Faculty and Academic Professionals"},O=({filters:n,API_URL:u,searchApiVersion:f,appPathFolder:y,deptIds:h,display:b,profileURLBase:g,searchType:s,grid:w})=>{const[T,a]=l.useState({}),[j,P]=l.useState(null),[R,m]=l.useState(!0),[_,S]=l.useState({}),D=6,t="faculty_rank",i=new URL(`${u}${f}${C[c.WEB_DIRECTORY_FACULTY_RANK].url}`);i.searchParams.append("sort_by",s),i.searchParams.append("dept_ids",h),i.searchParams.append("page","1"),i.searchParams.append("size","1");const x=async p=>{var N;const o=`${i}&rank_group=${p}`,L=await(await fetch(o)).json();return{rankGroup:p,length:(N=L.results)==null?void 0:N.length}};l.useEffect(()=>{const p={...n};p.deptIds=h.split(","),a(p);const o=Object.keys(F).map(x);Promise.all(o).then(d=>{const L=d.reduce((N,{rankGroup:W,length:Y})=>(Y>0&&(N[W]=F[W]),N),{});S(L),m(!1)})},[h,j]);const E={filters:n,API_URL:u,searchApiVersion:f,profileURLBase:g||"https://search.asu.edu",appPathFolder:y},v={[c.WEB_DIRECTORY_FACULTY_RANK]:{...C[c.WEB_DIRECTORY_FACULTY_RANK],...E}},I={faculty_rank:c.WEB_DIRECTORY_FACULTY_RANK};return R?e.jsx("div",{children:"Loading..."}):Object.keys(_).length?e.jsx(V,{onTabChange:P,children:Object.keys(_).map(p=>e.jsx(K,{id:`faculty-${p}`,title:F[p],children:e.jsx(B,{engine:v[I[s]],itemsPerPage:parseInt(b.profilesPerPage,10)||D,size:"large",sort:t,hidePaginator:b.usePager!=="1",filters:T,profilesToFilterOut:b.doNotDisplayProfiles,display:b,rankGroup:p.toString(),restClientTag:"asuis",grid:w})},p))}):e.jsxs("div",{children:["No faculty rank data found for department ",h]})};O.propTypes={deptIds:r.string,API_URL:r.string,searchApiVersion:r.string,searchType:r.string,profileURLBase:r.string,appPathFolder:r.string,display:r.shape({defaultSort:r.string,doNotDisplayProfiles:r.string,profilesPerPage:r.string,usePager:r.string}),filters:r.shape({employee:r.string,expertise:r.string,title:r.string,campuses:r.string}),grid:r.bool};O.__docgenInfo={description:`Prop types for FacultyRankTabPanels component.
@type {Object}
...
`,k=({filterLabel:n,choices:u=[],onChoose:f=()=>{},resetFilters:y=()=>{}})=>{const[h,b]=l.useState(null),[g,s]=l.useState(-1),[w,T]=l.useState(0),a=l.useRef(null),[j,P]=l.useState(0),[R,m]=l.useState(0);l.useEffect(()=>{const t=()=>{P(a.current.scrollLeft)};return a.current&&(a.current.addEventListener("scroll",t),t()),()=>{var i;return(i=a.current)==null?void 0:i.removeEventListener("scroll",t)}},[R]),l.useEffect(()=>{const t=()=>{a.current&&m(a.current.scrollWidth-a.current.offsetWidth)};return a.current&&(a.current.addEventListener("resize",t),t()),()=>{var i;return(i=a.current)==null?void 0:i.removeEventListener("resize",t)}},[]),l.useEffect(()=>{if(a.current){const t=a.current.children.length;T(t)}},[]);const _=t=>{var i;if(a.current){if(t.key==="ArrowRight"&&g+1<w){const x=(i=a.current)==null?void 0:i.children[g+1];s(g+1),x.focus()}else if(t.key==="ArrowLeft"&&g-1>=0){const x=a.current.children[g-1];s(g-1),x.focus()}}},S=t=>{b(t),f(t)},D=t=>{const i=a.current,x=i.scrollWidth-i.clientWidth;let v=i.scrollLeft+200*t;v=Math.max(0,Math.min(x,v)),i.scrollTo({left:v,behavior:"smooth"})};return e.jsxs(G,{className:"filter-container",children:[e.jsx("legend",{children:n}),e.jsxs("div",{className:"choices-wrapper",children:[e.jsx($,{hidePrev:j<=0,hideNext:j>=R-5,clickPrev:()=>{D(-1)},clickNext:()=>{D(1)}}),e.jsxs("div",{role:"radiogroup",tabIndex:0,onKeyDown:_,className:"choices-container",ref:a,"aria-label":`${n} filter options}`,children:[y&&e.jsx("button",{role:"radio",type:"button",onClick:t=>{t.preventDefault(),S(null),y()},tabIndex:-1,"aria-label":"All","aria-checked":h===null,className:"choice",children:"All"}),u.map(t=>e.jsx("button",{role:"radio",type:"button",onClick:i=>{i.preventDefault(),S(t)},"aria-checked":h===t,className:`${h===t?"selected":""} choice`,"aria-label":`Filter by ${t}`,tabIndex:-1,children:t},t))]})]})]})};k.propTypes={filterLabel:r.string.isRequired,choices:r.arrayOf(r.string).isRequired,onChoose:r.func.isRequired,resetFilters:r.func};k.__docgenInfo={description:`Prop types for FilterComponent component.
@type {Object}
