...
`,k=({filterLabel:n,choices:u=[],onChoose:f=()=>{},resetFilters:b=()=>{}})=>{const[h,y]=l.useState(null),[g,r]=l.useState(-1),[w,T]=l.useState(0),a=l.useRef(null),[S,P]=l.useState(0),[R,m]=l.useState(0);l.useEffect(()=>{const s=()=>{P(a.current.scrollLeft)};return a.current&&(a.current.addEventListener("scroll",s),s()),()=>{var i;return(i=a.current)==null?void 0:i.removeEventListener("scroll",s)}},[R]),l.useEffect(()=>{const s=()=>{a.current&&m(a.current.scrollWidth-a.current.offsetWidth)};return a.current&&(a.current.addEventListener("resize",s),s()),()=>{var i;return(i=a.current)==null?void 0:i.removeEventListener("resize",s)}},[]),l.useEffect(()=>{if(a.current){const s=a.current.children.length;T(s)}},[]);const _=s=>{var i;if(a.current){if(s.key==="ArrowRight"&&g+1<w){const x=(i=a.current)==null?void 0:i.children[g+1];r(g+1),x.focus()}else if(s.key==="ArrowLeft"&&g-1>=0){const x=a.current.children[g-1];r(g-1),x.focus()}}},j=s=>{y(s),f(s)},D=s=>{const i=a.current,x=i.scrollWidth-i.clientWidth;let v=i.scrollLeft+200*s;v=Math.max(0,Math.min(x,v)),i.scrollTo({left:v,behavior:"smooth"})};return e.jsxs(H,{className:"filter-container",children:[e.jsx("legend",{children:n}),e.jsxs("div",{className:"choices-wrapper",children:[e.jsx(K,{hidePrev:S<=0,hideNext:S>=R-5,clickPrev:()=>{D(-1)},clickNext:()=>{D(1)}}),e.jsxs("div",{role:"radiogroup",tabIndex:0,onKeyDown:_,className:"choices-container",ref:a,"aria-label":`${n} filter options}`,children:[b&&e.jsx("button",{role:"radio",type:"button",onClick:s=>{s.preventDefault(),j(null),b()},tabIndex:-1,"aria-label":"All","aria-checked":h===null,className:"choice",children:"All"}),u.map(s=>e.jsx("button",{role:"radio",type:"button",onClick:i=>{i.preventDefault(),j(s)},"aria-checked":h===s,className:`${h===s?"selected":""} choice`,"aria-label":`Filter by ${s}`,tabIndex:-1,children:s},s))]})]})]})};k.propTypes={filterLabel:t.string.isRequired,choices:t.arrayOf(t.string).isRequired,onChoose:t.func.isRequired,resetFilters:t.func};k.__docgenInfo={description:`Prop types for FilterComponent component.
@type {Object}
